...
namespace GraphomatDrawingLibUwp
{
    ...
    public sealed partial class DrawControl : UserControl
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine("SelectedOn: " + DateTime.Now.Millisecond);
        ...
        private List<GraphDrawing> childrenDrawing;
        ...
        public int SelectedGraphIndex
        {
            ...
            set
            {
                System.Diagnostics.Debug.WriteLine("Selected: " + DateTime.Now.Millisecond);
        ...
        public DrawControl()
        {
            ...
            childrenDrawing = new List<GraphDrawing>();
        }
        ...
        private void SetGraphDrawingList()
        {
            ...
            List<GraphDrawing> newChildrenDrawing = new List<GraphDrawing>();

            foreach (Graph graph in children)
            {
                int index = childrenDrawing.FindIndex(x => x.Graph == graph);

                if (index == -1) newChildrenDrawing.Add(new GraphDrawing(graph, ViewArgs));
        ...
        private void Control_PointerMoved(object sender, PointerRoutedEventArgs e)
        {
            ...
            lock (this)
            {
                if (pointerPoints.Count <= 1 || !isDrew) return;
        ...
        private void SetViewDimensionToAxesAndChildren()
        {
            ...
            foreach (GraphDrawing child in childrenDrawing)
            {
        ...
        {
            GraphDrawing tappedChild;

            bool findChild = GetNearestChildDrawingInRange(e.GetPosition(ccDraw).ToVector2(), out tappedChild);
            System.Diagnostics.Debug.WriteLine("Tapped: " + DateTime.Now.Millisecond);
            SelectedGraphIndex = childrenDrawing.IndexOf(tappedChild);
            ZoomToChild(tappedChild);

            if (!findChild) return;
            if (TappedCurve != null) TappedCurve(this, tappedChild.Graph);

            e.Handled = true;
        }

        private void ZoomToChild(GraphDrawing child)
        {
        ...
        private bool GetNearestChildDrawingInRange(Vector2 vector, out GraphDrawing nearestChildInRange)
        {
